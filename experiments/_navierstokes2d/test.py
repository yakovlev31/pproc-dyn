...
for i in tqdm(range(0, len(test_data))):
    ...
    for j in range(cfg.n_mc_samples):
        ...
        z_unif = odeint(model.dyn, z_0, t_unif, rtol=cfg.rtol, atol=cfg.atol, method=cfg.solver).transpose(0, 1)  # type: ignore
...
print("Num. parameters: ", utils.count_parameters(model))
