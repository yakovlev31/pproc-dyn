...
for i in tqdm(range(cfg.n_iters)):
    ...
    train_batch_inds = np.random.choice(
        len(train_data), size=cfg.batch_size, replace=False
    )
    ...
    if i % max(1, int(0.005 * cfg.n_iters)) == 0 or i == cfg.n_iters - 1:
        with torch.no_grad():
            ...
            val_batch_inds = np.random.choice(
                len(val_data), size=cfg.batch_size, replace=False
            )
            val_batch = [val_data[i] for i in val_batch_inds]

            obs_lik_val, process_lik_val, kl_qp_val, aux_val = model(
                val_batch, cfg, device
            )
