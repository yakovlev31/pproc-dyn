...
def create_argparser() -> argparse.ArgumentParser:
    ...
    # Data.
    parser.add_argument("--data_dir", type=str, default="./data/Burgers1D/medium_u_cont/", help="Path to the dataset.")
    ...
    parser.add_argument("--d_x", type=int, default=1, help="Spatial coordinates dimension")
    parser.add_argument("--d_z", type=int, default=64, help="Latent state dimension")
    ...
    parser.add_argument("--t_unif_res", type=int, default=50, help="Number of point in unfirom temporal grid used for intepolation.")
    parser.add_argument("--interp_method", type=str, default="linear", help="Latent state interpolation method (nearest/linear).")


    # Model (encoder).
    parser.add_argument("--d_model", type=int, default=128, help="Transformer stack feature dimension.")
    parser.add_argument("--n_attn_heads", type=int, default=4, help="Transformer stack attention heads.")
    parser.add_argument("--n_tf_layers", type=int, default=4, help="Number of layers in transformer stack.")
    ...
    parser.add_argument("--phi_latent_dim", type=int, default=368, help="Hidden layer dimension in state decoder.")


    # Model (lm).
    parser.add_argument("--lm_hid_layers", type=int, default=3, help="Number of hidden layers in intensity function.")
    parser.add_argument("--lm_latent_dim", type=int, default=368, help="Hidden layer dimension in intensity function.")


    # Training/validation/testing.
    parser.add_argument("--obs_lik_scaler", type=float, default=1.0, help="Scaler for observation likelihood term.")
    ...
    parser.add_argument("--n_mc_samples", type=int, default=1, help="Number of samples for Monte Carlo integration.")

    parser.add_argument("--model_dir", type=str, default="./models/Burgers1D/", help="Folder for saving/loading models.")
...
def create_datasets(cfg: SimpleNamespace) -> tuple[list[Trajectory], ...]:
    ...
    # Filter data.
    thresholds = {"coarse": 1, "medium": 1, "fine": 1}  # {"coarse": 5, "medium": 10, "fine": 20}
    threshold = next((thresholds[key] for key in thresholds if key in cfg.data_dir))
    filt_data = []
    for traj in tqdm(data, desc="Filtering data"):
        if sum(traj.ctx_mask) >= threshold and sum(traj.obs_mask) >= threshold:
...
def visualize(
    ...
    fig, ax = plt.subplots(1, 5, figsize=(4*5, 3), constrained_layout=True)

    im0 = ax[0].scatter(
        context[:, 0], context[:, 1:1+cfg.d_x],
        c=context[:, 1+cfg.d_x:],
        marker="X", s=30, cmap="coolwarm"
    )

    im1 = ax[1].tricontourf(
        observations[:, 0], observations[:, 1:1+cfg.d_x].ravel(), observations[:, 1+cfg.d_x:].ravel(),
        cmap='coolwarm'
    )
    ax[1].scatter(observations[:, 0], observations[:, 1:1+cfg.d_x].ravel(), c="k", marker=".", s=10, alpha=0.3)

    im2 = ax[2].tricontourf(
        observations[:, 0], observations[:, 1:1+cfg.d_x].ravel(), mu.ravel(),
        cmap='coolwarm'
    )
