...
def create_argparser() -> argparse.ArgumentParser:
    ...
    # Data.
    parser.add_argument("--data_dir", type=str, default="./data/ShallowWater2D/medium_u_cont/", help="Path to the dataset.")

    parser.add_argument("--T", type=float, default=1.0, help="Full time interval.")
    parser.add_argument("--t_ctx", type=float, default=0.1, help="Context time interval.")
    ...
    parser.add_argument("--dyn_latent_dim", type=int, default=256, help="Hidden layer dimension in dynamics function.")


    # Model (phi).
    parser.add_argument("--phi_hid_layers", type=int, default=3, help="Number of hidden layers in state decoder.")
    parser.add_argument("--phi_latent_dim", type=int, default=256, help="Hidden layer dimension in state decoder.")
    ...
    parser.add_argument("--model_dir", type=str, default="./models/ShallowWater2D/", help="Folder for saving/loading models.")
...
def create_datasets(cfg: SimpleNamespace) -> tuple[list[Trajectory], ...]:
    ...
    for path in tqdm(glob.glob(cfg.data_dir+"*.pkl"), desc="Reading data"):
        with open(path, "rb") as f:
            ...
            if traj[0].min() < 0 or traj[0].max() > 1:
                raise RuntimeError("Time points must be between 0 and 1.")
            if traj[1].min() < -2.5 or traj[1].max() > 2.5:
                raise RuntimeError("Spatial locations must be between -2.5 and 2.5.")
    ...
    for traj in tqdm(data, desc="Filtering data"):
        # if sum(traj.ctx_mask) >= 50 and sum(traj.ctx_mask) <= 400:
        if sum(traj.ctx_mask) >= 10:
